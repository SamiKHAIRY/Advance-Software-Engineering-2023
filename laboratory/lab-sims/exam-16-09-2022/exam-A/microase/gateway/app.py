...
@app.route("/exam/mulstring")
def mulstring():
    a = request.args.get('a')
    n = request.args.get('n', type=int)

    try:
        x = requests.get(EXAM_URL + f'/mulstring?a={a}&n={n}')
        x.raise_for_status()
        return x.json()
    except ConnectionError:
        return make_response('Mulstring service is down\n', 404)
    except HTTPError:
        return make_response('Invalid ciao input\n', 400)
